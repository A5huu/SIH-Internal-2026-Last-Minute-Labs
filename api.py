...
from backend.copilot_parser import parse_chartering_request
from backend.copilot import run_chartering_copilot
...
def parse_copilot_request(request: dict):
    ...
    return parse_chartering_request(text)


@router.post("/copilot/cargo/{cargo_id}")
def api_chartering_copilot(
    cargo_id: int,
    db: Session = Depends(get_db)
):
    try:
        return run_chartering_copilot(
            cargo_id=cargo_id,
            db=db
        )
...
# =============================================================================
# AI CHARTERING COPILOT - END-TO-END CHAT
# =============================================================================

class CopilotChatRequest(BaseModel):
    """Natural-language request sent to the AI Chartering Copilot."""
    message: str = Field(
        ...,
        min_length=5,
        description="Natural-language chartering requirement",
        json_schema_extra={
            "example": (
                "Find the best vessel for 75,000 MT coking coal from Australia "
                "to Paradip between 12 and 23 November. "
...
def copilot_chat(
    ...
    """
    End-to-end AI Chartering Copilot.
    ...
    """
    try:
        # 1. Parse the natural-language chartering request.
        message = req.message.strip()
        parsed = parse_chartering_request(message)

        if not parsed.get("parsed"):
            raise HTTPException(
                status_code=400,
                detail={
                    "message": "I could not understand the chartering requirement.",
                    "parsed_request": parsed
                }
            )

        # 2. Make sure all information required by the cargo/recommendation
        ...
        if missing_fields:
            raise HTTPException(
                status_code=400,
                detail={
                    "message": "Some chartering details are missing.",
                    "missing_fields": missing_fields,
                    "parsed_request": parsed
                }
            )

        # 3. Resolve the natural-language destination to a database Port.
        destination_name = str(parsed["destination"]).strip()

        port = (
            db.query(Port)
            .filter(func.lower(Port.name) == destination_name.lower())
            .first()
        )

        # If an exact match is not found, try a partial name match.
        if not port:
            port = (
                db.query(Port)
                .filter(func.lower(Port.name).contains(destination_name.lower()))
                .first()
            )
        ...
        if not port:
            raise HTTPException(
                status_code=404,
                detail=f"Destination port '{destination_name}' was not found."
            )

        # 4. Infer the commercial contract preference from the request.
        #    Spot remains the safe default when the user does not specify CoA.
        message_lower = message.lower()

        wants_coa = bool(
            re.search(r"\bcoa\b", message_lower)
            or "contract of affreightment" in message_lower
            or "long term contract" in message_lower
            or "long-term contract" in message_lower
            or "long term" in message_lower
            or "long-term" in message_lower
        )

        contract_preference = "CoA" if wants_coa else "Spot"

        # 5. Convert the parsed requirement into the existing Cargo model.
        cargo_payload = {
            "cargo_type": parsed["cargo_type"],
            "quantity": parsed["quantity"],
            "origin": parsed["origin"],
            "destination_port_id": port.id,
            "laycan_start": parsed["laycan_start"],
            "laycan_end": parsed["laycan_end"],
            "contract_preference": contract_preference
        }

        cargo = logic.create_cargo(cargo_payload, db)

        # 6. Run the existing vessel matching + freight/risk/recommendation
        #    engine against the newly created cargo requirement.
        recommendation = run_chartering_copilot(
            cargo_id=cargo["id"],
            db=db
        )

        # 7. Return one frontend-friendly response containing every stage.
        return {
            "success": True,
            "message": message,
            "parsed_request": parsed,
            ...
            "copilot_status": "Recommendation generated successfully."
        }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Copilot processing failed: {str(exc)}"
        )
